    ...
    <div className="px-4 py-4">
      <div className="mb-8">
        ...
        <textarea
          ...
          rows={10}
          className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:border-slate-400 resize-none leading-relaxed"
        />
        <div className="flex items-center justify-between mt-1.5">
          <span className="text-xs text-slate-400">{savedText}</span>
          <span className="text-xs text-slate-300">{notes.length} chars</span>
        </div>
      </div>
      ...
      <div>
        ...
        {links.length === 0 && (
          <p className="text-sm text-slate-400 italic mb-4">Nothing here yet — add something +</p>
        )}
        ...
        <div className="flex flex-col gap-2 mb-4">
          {links.map(link => (
            <div
              key={link.id}
              className="flex items-center gap-3 border border-slate-200 rounded-xl px-4 py-3 bg-white group"
            >
              <a
                href={link.url}
                target="_blank"
              ...
              >
                ...
                <span className="text-sm font-medium text-slate-700 hover:text-slate-900 truncate transition-colors">
                  {link.label}
                </span>
              </a>
              <button
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          <input
            ...
            className="w-full px-4 py-3 text-sm outline-none border-b border-slate-100 text-slate-700 placeholder:text-slate-400"
          />
          <input
            type="url"
            value={newUrl}
            ...
            className="w-full px-4 py-3 text-sm outline-none text-slate-700 placeholder:text-slate-400"
          />
